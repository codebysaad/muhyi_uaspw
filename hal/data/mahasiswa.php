...
function submit(x) {
    if (x == 'add') {
        ...
        $('#modal_add .modal-title').html('Add New Mahasiswa');
        $('#btn-ubah').hide();
        $('#btn-add').show();
    } else {
        $('#modal_add .modal-title').html('Edit Mahasiswa')
        $('#btn-add').hide();
        $('#btn-ubah').show();
        ...
        $.ajax({
            ...
            url: '<?=$base_url;?>proses/view_mhs.php',
            dataType: 'json',
            success: function(data) {
                $('[name="idmhs"]').val(data.idmahasiswa);
        ...
        <div class="col-md-12">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title">Master Data Mahasiswa</h3>
                    <ul class="panel-controls">
                        ...
                        <li data-toggle="tooltip" data-placement="top" title="Import Mahasiswa"><a href="#"
                        ...
                        <li data-toggle="tooltip" data-placement="top" title="Aktifkan Akun"><a
                                href="<?=$base_url;?>?aktifkan_keys&lvl=mahasiswa"><span class="fa fa-key"></span></a>
                        </li>
                ...
                <div class="panel-body table-responsive">
                    <table class="table datatable">
                        ...
                        <tbody>
                            <?php
                            $n=1;
                            $query = mysqli_query($con,"SELECT * FROM mahasiswa ORDER BY nim ASC")or die(mysqli_error($con));
                            while($row = mysqli_fetch_array($query)):
                            ?>
                            <tr>
                                <td><?=$n++.'.';?></td>
                                <td><a href="#modal_add" data-toggle="modal"
                                        onclick="submit(<?=$row['idmahasiswa'];?>)"><i class="fa fa-edit"></i></a></td>
                                <td><?=$row['nim'];?></td>
                                <td><?=$row['nama_lengkap'];?></td>
                                <td><?=$row['tempat_lahir'].', '.$row['tanggal_lahir'];?></td>
                                <td><?=$row['jk'];?></td>
                                <td><?=$row['jenjang'];?></td>
                                <td><?=$row['program_studi']==1?'Teknik Informatika':'Teknik Komputer';?></td>
                                <td><a href="<?=$base_url;?>proses/mhs.php?id=<?=$row['idmahasiswa'];?>"
...
<div class="modal" id="modal_add" tabindex="-1" role="dialog" aria-labelledby="largeModalHead" aria-hidden="true"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="<?=$base_url;?>proses/mhs.php" method="post" id="form-mahasiswa">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-4" style="margin-bottom:15px;">
                            <div class="form-group">
                                <label class="control-label">Program Studi<span style="color:red;">*</span></label>
...
<div class="modal" id="modal_import" tabindex="-1" role="dialog" aria-labelledby="largeModalHead" aria-hidden="true"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="<?=$base_url;?>proses/import_mhs.php" method="post">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"><span
                            aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                    <h4 class="modal-title" id="largeModalHead">Import Mahasiswa</h4>
                </div>
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-12" style="margin-bottom:15px;">
                            ...
                            <ol>
                                <li>Copy dan paste <strong>[NIM] [NAMA LENGKAP] [TEMPAT LAHIR] [TANGGAL LAHIR] [JENIS
                                        KELAMIN] [JENJANG] [PROGRAM STUDI]</strong> dari Ms. Excel pada Text Area
                                    dibawah.</li>
                                <li>Kolom <strong>JENIS KELAMIN</strong> diisi huruf <strong>"L"</strong> jika Laki-Laki
                                    dan <strong>"P"</strong> jika Perempuan.</li>
                                <li>Kolom <strong>PROGRAM STUDI</strong> diisi angka <strong>"1"</strong> jika Teknik
                                    Informatika dan <strong>"2"</strong> jika Teknik Komputer.</li>
                                <li>Kolom <strong>TANGGAL LAHIR</strong> diisi dengan format
                                    <strong>"YYYY-MM-DD"</strong>. Contoh :
                                    <strong>1991-03-15</strong></li>
                            </ol>
                            <div class="form-group">
                                <textarea class="form-control" rows="15" name="mhs" placeholder="Paste here..."
                                    required></textarea>
                            </div>
                        </div>
                    </div>
...
<div class="modal" id="modal_delete" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="<?=$base_url;?>proses/mhs.php" method="post">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"><span
                            aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
